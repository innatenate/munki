...
def currentForecast():
    ...
    day = {
        ...
        'detail': day['conditions']
    }
    day['takeaway'] = uni.choose(["favorable", "decent", "good", "okay"])

    if day['detail'] == 'Rain':
        day['takeaway'] = uni.choose(['rainy', 'stormy', 'inclement', 'damp'])
    elif day['detail'] == 'Cloudy' or day['detail'] == 'Partly Cloudy':
        day['takeaway'] = uni.choose(['cloudy', 'melancholic', 'overcast', 'overclouded'])
    ...
    if day['humidity'] > 80:
        day['notes'] += uni.choose([
            f"Also, humidity for the area is reading rather high, at a {day['humidity']}%.",
            f"I am seeing a humidity reading of {day['humidity']}%.",
            f"I've also noticed a rather high humidity reading."
        ])
    difference = day['temp'] - day['fl']

    phrase = uni.choose([
        f"The current conditions for today are {day['takeaway']} with temperatures around {day['temp']}°**.",
        f"Currently, it seems {day['takeaway']} with temperatures currently at {day['temp']}°**.",
        f"The current forecast for today is {day['detail']} with temperatures at {day['temp']}°**."
    ])
...
def dataTomProcess():
    ...
    if returnDay['details']['pressure'] >= highPressureThresh:
        ...
        returnDay['notes']['pressure'] = 'low'
    universal.weather['pastDay'] = returnDay
    return returnDay
...
def forecastDetail(info):
    ...
    if context == "differentday":
        trueDay = data['day']
        if universal.weather['7week']:
            noResult = True
            for day in universal.weather['7week']:
                if 'day' in day:
                    if day == trueDay:
                        noResult = False
                        if detail == "temperatures":
                            return universal.weather['7week'][day]['temps']
                        elif detail == "pressure": 
                            return universal.weather['7week'][day]['details']['pressure']
                        elif detail == "humidity": 
                            return universal.weather['7week'][day]['details']['humidity']
                        elif detail == "dewpoint": 
                            return universal.weather['7week'][day]['details']['dp']
                        elif detail == "clouds": 
                            return universal.weather['7week'][day]['details']['clouds']
                        elif detail == "pop": 
                            return universal.weather['7week'][day]['details']['pop']
                        elif detail == "poptype": 
                            return universal.weather['7week'][day]['details']['poptype']
                        elif detail == "weather": 
                            return universal.weather['7week'][day]['forecast']
                        else:
                            noResult = True
            if noResult == True:
...
def update():
    print("updating weather values")
    t5day = data5Process()
    t7day = data7Process()
    current = currentForecast()
    tomorrow = dataTomProcess()
    currentTime = time.time()
    week = {
        "dt": currentTime,
        "weather": t5day}                                ## Convert to dict with weather = procweek and dt = now
    if universal.weather['past5week']:
        weekTime = universal.weather['past5week']['dt']
        if (weekTime + 604800) <= week['dt']:
            universal.weather['past5week'] = week
    ...
    week = {
        "dt": currentTime,
        "weather": t7day}                                ## Convert to dict with weather = procweek and dt = now
    if universal.weather['past7week']:
        weekTime = universal.weather['past7week']['dt']
        if (weekTime + 604800) <= week['dt']:
            universal.weather['past7week'] = week
